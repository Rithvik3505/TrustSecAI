...
The original full LoRA run completed for all 76 held-out examples, but every generation hit the `max_new_tokens=900` cap. Outputs often began as JSON and were truncated because the model copied long provenance arrays. Prefer the compact rerun commands below before using results for reporting.
...
```bash
python -m src.llm.evaluation.run_lora_inference --model-mode lora --limit 3 --max-new-tokens 1200 --compact-output-instruction --output-suffix compact_smoke
```

Evaluate compact smoke output:

```bash
python -m src.llm.evaluation.evaluate_generations --lora-file artifacts/evaluation/lora_v1/lora_generations_compact_smoke.jsonl
```
...
Evaluate compact full LoRA output:

```bash
python -m src.llm.evaluation.evaluate_generations --lora-file artifacts/evaluation/lora_v1/lora_generations_compact.jsonl
```
